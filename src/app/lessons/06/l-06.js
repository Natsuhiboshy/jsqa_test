...
(function () {
	...
	var cube = function (num){
		if (typeof(num)!= 'number'){
			console.log('Please prowide input with type number.');
			return;
		}
		else {
			return num*num;
	...
	console.log('\nPlease implement this task');
	function1 = function () {
		global1 = 'LOCAL1'; //переприсвоение глобальной переменной
		global2 = 'LOCAL2'; //выполнилось переприсвоение глобальной переменной
		console.log('log from function scope: \n', global1, '\n', global2); // output: LOCAL1, LOCAL2
	};
...
function blackBox (hidden){
 ...
 let Hidden = 'secret';
 let resuler = blackBox (Hidden);
 let output = resuler();
console.log('Function blackBox received value', output);
